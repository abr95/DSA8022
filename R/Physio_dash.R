...
dataSimulationSidebar <- function(
) {
  sidebarPanel(
    sliderInput(
      inputId = "data_simulation_duration",
    ...
    sliderInput(
      inputId = "data_simulation_autocorrelation",
    ...
    actionButton(
      inputId = "data_simulation_simulate",
      label = "Simulate data"
    )
  )
}

dataSimulationMainPanel <- function(
) {
  mainPanel(
    tags$p("first two rows of the Heartrate file"),
    tableOutput(outputId = "HR_simulate_table"),
    tags$p("first two rows of the ECG file"),
    tableOutput(outputId = "ECG_simulate_table"),
    tags$p("first two rows of the Breathingrate file"),
    tableOutput(outputId = "BR_simulate_table"),
    tags$p("first two rows of the Skintemperature file"),
    tableOutput(outputId = "ST_simulate_table"),
    tags$p("first two rows of the GSR file"),
    tableOutput(outputId = "GSR_simulate_table")
  )
}

## input tab stuff

apiConnectTabPanel <- function(
) {
  tabPanel(
    title = "API connect",
    h2("Not Implemented")
...
dataSimulationTabPanel <- function(
) {
  tabPanel(
    title = "Data Simulation",
    sidebarLayout(
      sidebarPanel = dataSimulationSidebar(),
      mainPanel = dataSimulationMainPanel()
    )
  )
}
...
heartRateSidebar <- function(
) {
  sidebarPanel(
    tags$p("Evolution of Heart Rate (HR) according to time")
  )
}

heartRateMainPanel <- function(
) {
  mainPanel(
    plotOutput(
      outputId = "hr_plot"
...
heartRateTabPanel <- function(
) {
  tabPanel(
    title = "Heart Rate",
    sidebarLayout(
      sidebarPanel = heartRateSidebar(),
      mainPanel = heartRateMainPanel()
    )
  )
}

electrocardiogramSidebar <- function(
) {
  tags$p("Evolution of ElectroCardioGram (ECG) according to time and Heart Rate Variability (HRV) analysis")
  sidebarPanel(
    sliderInput(
      inputId = "ecg_r_threshold",
      label = "Threshold to identify ECG R peaks:",
      min = -2,
      max = 2,
      value = 0.7
    ),
    sliderInput(
      inputId = "ecg_window_size",
      label = "Size of window for calculations (seconds):",
      min = 100,
      max = 1000,
      value = 100
    ),
    sliderInput(
      inputId = "ecg_window_displacement",
      label = "Displacement of window for calcaulations (seconds):",
      min = 10,
      max = 100,
      value = 30
    ),
    sliderInput(
      inputId = "ecg_hrv_length",
      label = "Length of the HRV (high frequency) analysis:",
      min = 100,
      max = 1000,
      value = 100
    ),
    selectInput(
      inputId = "ecg_wavelet",
      label = "Mother wavelt used to calculate the spectrogram:",
      choices = c(
        "fourier",
        "wavelet"
      ),
    ...
    actionButton(
      inputId = "hrv_spectrum",
      label = "Display HRV spectrum plot"
    ),
    actionButton(
      inputId = "hrv_frequency",
      label = "Display HRV frequency plot"
    ),
    actionButton(
      inputId = "hrv_poincare",
      label = "Display HRV Poincare plot"
    ),
    plotOutput(outputId = "sidebar_plot")
  )
}

electrocardiogramMainPanel <- function(
) {
  mainPanel(
    plotOutput(outputId = "ecg_ecg"),
    plotOutput(outputId = "ecg_rr"),
    plotOutput(outputId = "ecg_hr")
  )
}

electrocardiogramTabPanel <- function(
) {
  tabPanel(
    title = "ElectroCardioGram",
    sidebarLayout(
      sidebarPanel = electrocardiogramSidebar(),
      mainPanel = electrocardiogramMainPanel()
    )
  )
}

breathingRateSidebar <- function(
) {
  sidebarPanel(
    tags$p("Evolution of Breathing Rate (BR) according to time")
  )
}

breathingRateMainPanel <- function(
) {
  mainPanel(
    plotOutput(outputId = "br_plot"),
  )
}


breathingRateTabPanel <- function(
) {
  tabPanel(
    title = "Breathing Rate",
    sidebarLayout(
      sidebarPanel = breathingRateSidebar(),
      mainPanel = breathingRateMainPanel()
    )
  )
}

skinTemperatureTabPanel <- function(
) {
  tabPanel(
...
galvanicSkinResponseTabPanel <- function(
) {
  tabPanel(
...
inputTabPanel <- function(
  ...
  tabPanel(
    ...
    tabsetPanel(
      apiConnectTabPanel(),
      dataUploadTabPanel(),
      dataSimulationTabPanel()
    )
  )
}

featuresTabPanel <- function(
) {
  tabPanel(
    title = "Features",
    tabsetPanel(
      heartRateTabPanel(),
      electrocardiogramTabPanel(),
      breathingRateTabPanel(),
      skinTemperatureTabPanel(),
      galvanicSkinResponseTabPanel()
    )
  )
}

biometricsTabPanel <- function(
) {
  tabPanel(
    title = "Biometrics",
    h2("TODO")
